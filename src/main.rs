...
mod app {
    ...
    use crate::audio::dac_output::DACOutput;
    ...
    pub fn main() -> anyhow::Result<()> {
        ...
        let audio_out = DACOutput::new();
        let decoder = DecoderTask::spawn(audio_out)?;
        ...
        //
        // Order matters for SDP. iPhone caches the SDP record set
        // from the FIRST connection — any profile not registered
        // before GAP makes us discoverable won't appear in that
        // cache, and the peer will refuse to open a channel for
        // it on later sessions.
        //
        // So: profile registration first (A2DP), GAP last
        // to flip the discoverable bit only once SDP is complete.
